...
async function main() {
  for (const [status, outcome] of [['Approved', 'SUCCESS'], ['Pending', 'PENDING'], ['Cancelled', 'REJECTED']] as const) {
    ...
    assert.equal(requests[0].options.path, '/api/airtime.php');
  ...
  assert.equal(query.requests[0].options.path, '/api/query_airtime.php');
  ...
  assert.equal(plans.requests[0].options.path, '/api/databundles.php');
  ...
  assert.equal(data.requests[0].options.path, '/api/data.php');
